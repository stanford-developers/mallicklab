...
STYLE = '''
  /* BEGIN index-fixes (generated by build_slider.py) */
  /* ---- slider controls -------------------------------------------------
  ...
  /* ---- header bar --------------------------------------------------
     custom_css_*.css pads #jm-bar-in 25px left and right on every page, but
     the theme's more specific `#jm-bar.noheader #jm-bar-in` zeroes it again.
     Inner pages carry .noheader; the home page can't, because it has the
     #jm-header slider region that tucks up into the bar. The result is that
     the home page has 50px less usable width and its nav wraps below the logo
     while every other page still fits. Zero it here for the home bar only.
...
def main():
    ...
    # --- slideshow timing ----------------------------------------------
    # The MooTools `new DJImageTabber(...)` call near the top of the page never
    # initialises in the static mirror; the slider is actually driven by the
    # hand-written script at the end of <body>. Its DELAY/DURATION are the ones
    # that matter. The dead options are kept in sync anyway so that nobody
    # later edits the wrong number and concludes it does nothing.
    page, n1 = re.subn(r'(var DELAY\s*=\s*)\d+',    r'\g<1>' + str(DELAY_MS), page)
    page, n2 = re.subn(r'(var DURATION\s*=\s*)\d+', r'\g<1>' + str(DURATION_MS), page)
    if not (n1 and n2):
        sys.exit('could not find DELAY/DURATION in the inline slider script')
    page = re.sub(r'(duration:\s*)\d+', r'\g<1>' + str(DURATION_MS), page)
    page = re.sub(r'(delay:\s*)\d+',    r'\g<1>' + str(DELAY_MS), page)

    # The inline script positions the active-tab indicator with a hard-coded
    # 80px pitch, which no longer matches now that the tabs divide the strip
    # evenly. Measure the tab instead.
    page = page.replace(
        "if (ind) ind.style.top = (current * 80) + 'px';",
        "if (ind && tabs[current]) ind.style.top = tabs[current].offsetTop + 'px';")

    # --- control CSS (idempotent) --------------------------------------
    page = re.sub(r'\n  /\* BEGIN (?:slider-controls|index-fixes).*?/\* END (?:slider-controls|index-fixes) \*/\n',
                  '', page, flags=re.S)
